...
let rec ocaml_value_of_json ~typ = function
  | (`Bool _ | `Float _ | `Int _ | `Null) as json ->
    ( match typ with
    | Some String -> sprintf "\\\"%s\\\"" (Yojson.Basic.to_string json)
...
let rec process_schema_type state ~ancestors (input_schema : schema) =
  ...
  | Some _, Some typ ->
    failwith
      (Printf.sprintf "only string enums are supported : on field %s got typ %s" (Option.value ~default:"" schema.title)
         (Json_schema_j.string_of_typ typ)
      )
  | None, _ ->
...
and process_object_type state ~ancestors schema =
  ...
  let make_record_field (field_name, schema_or_ref) =
    ...
    | Obj { default = Some default; enum; typ; _ }, _ ->
      sprintf "  ~%s %s <ocaml default=\"%s\">: %s;" record_field_name doc
        (make_atd_default_value ~typ enum default)
        type_
